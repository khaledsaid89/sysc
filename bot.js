const Discord = require('discord.js');  //Khaled#4867
const client = new Discord.Client();  /Khaled#4867
const moment = require('moment');
const zalgo = require('zalgolize');  //Khaled#4867
const math = require('math-expression-evaluator');  Khaled#4867
const figlet = require('figlet');   //Khaled#4867
const fs = require('fs');  //Khaled#4867
const ms = require('ms');  //Khaled#4867
const prefix = '-'
...
client.on('message', msg => {  //WESO#0001
  ...
  client.on('message', msg => {  //WESO#0001
   ...
   client.on('message', msg => {  //WESO#0001
    if (msg.content === 'لسلام عليكم') {  //WESO#0001
      msg.reply('**وعليكم السرلام :heart: **');  //WESO#0001
    }
  });
  ...
  message.channel.send(embed);
   ...
   //WESO#0001  //WESO#0001  //WESO#0001  //WESO#0001  //WESO#0001
      ...
      let awaiting = await message.channel.awaitMessages(r => r.author.id === mention.id, {max: 1, time: 20000, errors:['time']}).then(collected => {
        ...
        if(collected.first().content === 'نعم') {  //WESO#0001
          ...
          system[author.id].members += 1;  ///Khaled#4867
 
 
          system[mention.id] = { ///Khaled#4867
            clan: clan,
            joinedAt: new Date().toLocaleString(), ///Khaled#4867
            clanLevel: 0,
            creator: clans[clan].creator  //Khaled#4867
          };
        ...
        } else if(collected.first().content !== 'نعم' && collected.first().content !== 'لا'){  //Khaled
          return message.channel.send('**# يجب عليك كتابة `نعم` أو `لا`**');  //Khaled#4867
        }  //WESO#0001
      });  //WESO#0001
    }  //WESO#0001
    if(args[1] && args[1] === 'stats') {
      ...
      let embed = new Discord.RichEmbed()  //WESO#0001
        ...
        embed.addField('» تاريخ عمل الكلان', clans[clan].createdAt, true);  //Khaled
        embed.addField('» تاريخ دخول الكلان', system[author.id].joinedAt, true)  //Khaled
        embed.addField('» صاحب الكلان', `<@${clans[clan].creator}>`, true);  //Khaled
        embed.addField('» لفل الكلان', clans[clan].level, true);  //khaled
        embed.addField('» عدد اعضاء الكلان', clans[clan].members.length, true);  //khaled
        embed.addField('» عدد التقديمات للكلان', clans[clan].applylist.length, true);  //Khaled
        embed.addField('» عدد الادمنية بالكلان', clans[clan].admins.length, true);  //Khaled
        embed.addField('» اعضاء الكلان', `${prefix}clan list || يظهرلك رسالة بها اعضاء الكلان`);  //Khaled
 
    }
    if(args[1] && args[1] === 'join') {  //Khaled  //Khaled
   ...
   //WESO#0001
    ...
    if(args[1] && args[1] === 'promote') {
      ...
      if(!clans[system[author.id].clan].members.includes(mention.id)) return message.channel.send("**# هذا الشخص ليس بالكلان**");  //Khaled
      if(clans[system[author.id].clan].admins.includes(mention.id)) return message.channel.send("**# هذا العضو لديه ادمن بالفعل**");  //Khaled
      ...
      mention.send(`**# \`${system[author.id].clan}\`, لقد تم ترقيتك الى ادمن**`).catch();  //Khaled
      message.channel.send(`**# \`${mention.username}\`, لقد تم ترقية العضو الى رتبة ادمن**`); //Khaled
    if(args[1] && args[1] === 'demote') {  //Khaled
      let mention = message.mentions.users.first();  //Khaled
      if(system[author.id].clan === 'None') return message.channel.send("**# يجب ان تكون بكلان لأستخدام هذا الأمر**");  //Khaled
      if(clans[system[author.id].clan].creator !== message.author.id) return message.channel.send("**# هذا الأمر لضاحب الكلان فقط**");  //Khaled
      if(!mention) return message.channel.send("**# يجب عليك منشنة عضو بالكلان لأعطائه الترقية**");  //Khaled
      if(!system[mention.id]) system[mention.id] = {clan: 'None',joinedAt: new Date().toLocaleString() ,clanLevel: 0};  //Khaled
      if(system[mention.id].clan === 'None') return message.channel.send("**# هذا الشخص ليس بكلان**");  Khaled
      if(!clans[system[author.id].clan].members.includes(mention.id)) return message.channel.send("**# هذا الشخص ليس بالكلان**");  //Khaled
      if(!clans[system[author.id].clan].admins.includes(mention.id)) return message.channel.send("**# هذا الشخص ليس ادمن بالكلان**");  //Khaled
      if(mention.id === message.author.id) return message.channel.send("**# لا يمكنك اعطاء نفسك ترقية**");  //WESO#0001
   //WESO#0001
      clans[system[author.id].clan].admins.shift(mention.id);  //Khaled
...
client.on("message", message => {
 ...
 @everyone 
✧▬▬▬▬▬▬✦ஜ۩۞۩ஜ✧▬▬▬▬▬▬✦
:shield: | Khaled Servar  Official Discord Rules | :shield:
✧▬▬▬▬▬▬✦ஜ۩۞۩ஜ✧▬▬▬▬▬▬✦
:no_entry_sign: | 1 = ممنوع نشر روابط في العام
:no_entry_sign: | 2 = ممنوع السب
:no_entry_sign: | 3 = ممنوع السبام
:no_entry_sign: | 4 = ممنوع استخدام مغير صوت
:no_entry_sign: | 5 = ممنوع الازعاج في الرومات الصوتيه
✧▬▬▬▬▬▬✦ஜ۩۞۩ஜ✧▬▬▬▬▬▬✦
:scroll:  | Admin's Rules | :scroll: 
✧▬▬▬▬▬▬✦ஜ۩۞۩ஜ✧▬▬▬▬▬▬✦
:no_entry_sign: | 1 = ممنوع اعطاء الرتب الأداريه لأي احد
:no_entry_sign: | 2 = ممنوع صناعه رومات قبل معرفه الاونر
:no_entry_sign: | 3 = ممنوع اضافه رتب قبل معرفه الاونر
:no_entry_sign: | 4 = ممنوع الاسائه للرتبه
:no_entry_sign: | 5 = ممنوع السب او السبام
✧▬▬▬▬▬▬✦ஜ۩۞۩ஜ✧▬▬▬▬▬▬✦
كل ما سبق يؤدي الى سحب الرتبه لأيام محدده :wink:
✧▬▬▬▬▬▬✦ஜ۩۞۩ஜ✧▬▬▬▬▬▬✦
:sparkles: | The End | :sparkles:
